...
torchrun --standalone --nproc_per_node=2 -m scripts.base_train -- --depth=26 --target-param-data-ratio=8.25 --device-batch-size=1 --fp8 --run=$WANDB_RUN
# evaluate the model: CORE metric, BPB on train/val, and draw samples
torchrun --standalone --nproc_per_node=2 -m scripts.base_eval -- --device-batch-size=1
...
torchrun --standalone --nproc_per_node=2 -m scripts.chat_sft -- --device-batch-size=2 --run=$WANDB_RUN
